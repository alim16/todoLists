...
class Assignment
  ...
  def create_user(params={})
	  ...
	  user1.save
	 # return User.find_user_byname(username) by me
  end
  ...
  def create_todolist(params={})
      ...
      # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
	   params2 = {}
	   params2[:list_name] = params[:name]
	   params2[:list_due_date] = params[:due_date]
	    list = TodoList.new(params2)
	  list.save
  end
  ...
  def find_alllists(offset, limit)
	  ...
	  lists = TodoList.limit(limit).offset(offset).order(list_due_date: :desc)
  end
  ...
  def update_password(id, password_digest)
	 ...
	 user = User.find_by(id: id)
	 user.update_attribute(:password_digest, password_digest)
  end
  ...
  def update_listname(id, name)
      ...
      # (no return is required)
	  list = TodoList.find_by(id: id)
	  	 list.update_attribute(:list_name, name)
  end 
